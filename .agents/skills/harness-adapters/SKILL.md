...
description: >-
  ...
  Contains verified facts for claude, codex, opencode, pi, pi-signed, grok, kimi, cursor, gemini, muse, rovo, omp, and cortex.
...
Muse, Gemini, Rovo, and Cortex are verified only for crewmate and scout work, never a secondmate or primary.
...
Cortex publishes `CORTEX_SESSION_ID` and `CORTEX_TASK_CONTEXT_ID` on its tool subprocesses, checked ahead of `CLAUDECODE` because cortex does not clear an inherited one, with the anchored process name `cortex` as ancestry evidence.
...
Kimi establishes its unsupported primary boundary in its selected harness reference; Muse, Gemini, Rovo, and Cortex follow Non-negotiable safety above.
...
```json harness-adapter-routing-v1
{
  ...
  "harnesses": {
    ...
    "cortex": "references/harness/cortex.md"
  }
}
```
