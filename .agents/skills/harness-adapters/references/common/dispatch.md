...
Verify detection in `../../../bin/fm-harness.sh`, launch in `../../../bin/fm-spawn.sh`, busy state in `../../../bin/fm-busy-lib.sh`, shared composer behavior in `../../../bin/fm-composer-lib.sh`, lifecycle in `../../../bin/fm-control-lib.sh`, and process-name liveness in `../../../bin/fm-harness-process-lib.sh`, whose `fm_harness_classify_process_name` is the single owner both the tmux and herdr adapters delegate to, when secondmate use is supported.
