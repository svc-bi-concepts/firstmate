...
| Hook config | `--config <settings.json>` is NOT a hook source - a verified NEGATIVE, and the one thing the flag's existence would otherwise lead you to assume. Hooks come only from the fixed loader path set, so firstmate's ride the worktree's `.cortex/settings.local.json`. See "Hooks do not ride `--config`" below. |
