...
Herdr's installed build ships no cortex integration, so its native agent read cannot tell a live cortex pane from an empty one: steering a cortex worker on herdr still lands, while every `fm-control.sh` lifecycle verb refuses there rather than acting on that blind read ([`herdr-backend.md`](herdr-backend.md#restart-and-liveness-behavior) owns the classification rule).
