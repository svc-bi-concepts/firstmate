...
rovo is likewise verified for crewmate and scout launches ONLY, refused for a secondmate for the same reason - no turn-end hook and no primary supervision protocol; [`docs/verification/rovo.md`](verification/rovo.md) owns that evidence, including the OAuth token's silent background refresh from a stored refresh token and both tmux and herdr pane liveness (herdr placement is verified live, and the Herdr-side agent-detection gap is now guarded by the coverage-plus-process-attribution rule in [`herdr-backend.md`](herdr-backend.md#restart-and-liveness-behavior)).
cortex (Snowflake Cortex Code) is likewise verified for crewmate and scout launches ONLY, refused for a secondmate for the same reason - no wake protocol and no turn-end guard adapter; [`docs/verification/cortex.md`](verification/cortex.md) owns that evidence, including the live tmux and Herdr runs and why its busy-state and turn-end hooks ride the worktree's own `.cortex/settings.local.json` instead of a firstmate-owned file outside it.
cortex also needs a credential before spawning: the operator's `~/.snowflake/cortex/settings.json` supplies `cortexAgentConnectionName`, and a worker cannot reach Snowflake without it.
Herdr's installed build ships no cortex integration, so its native agent read cannot tell a live cortex pane from an empty one; firstmate reads Herdr's own reported integration coverage and attributes an uncovered harness's pane from its foreground process instead, so steering and the `fm-control.sh` lifecycle verbs all work for a cortex worker on herdr ([`herdr-backend.md`](herdr-backend.md#restart-and-liveness-behavior) owns the classification rule and its uncertainty cases).
...
[`validation-agent.md`](validation-agent.md) owns which agent that validation pipeline itself uses, including the ACP bridge for a harness no-mistakes cannot drive natively, and it is a separate setting from this home's crew harness and dispatch profiles.
