...
The injection sits at the unconditional pre-launch export site, so it covers ship and scout spawns on every verified adapter, plus Secondmate spawns on every adapter verified for that kind - the crewmate/scout-only adapters are exactly the ones excluded from the Secondmate case, and [`configuration.md`](configuration.md#harness-support) owns both sets.
